...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "dialog-overlay-motion fixed inset-0 z-[10000] bg-black/80",
      className
    )}
    {...props}
  />
...
>(({ className, children, hideCloseButton = false, onPointerDownOutside, onInteractOutside, ...props }, ref) => {
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "dialog-content-motion fixed left-1/2 top-1/2 z-[10000] grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-2xl ring-1 ring-black/10 opacity-100 max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-h-[calc(100vh-4rem)] pointer-events-auto dark:border-slate-800 dark:bg-slate-900 dark:text-white dark:ring-white/10",
        className
      )}
